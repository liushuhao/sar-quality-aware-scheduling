#!/usr/bin/env python3
"""
MOEA-3 ablation variant D: NO PHYSICS (f2/f3 = 1, no geometric info).

Differences from baseline:
  - f2 = Σ 1.0      (constant; no geometry)
  - f3 = Σ 1.0      (constant; no geometry)
  - Only f1 (profit) is meaningful; f2/f3 are "free" placeholders

Tests whether geometric modeling in f2/f3 is necessary at all.
With constant f2/f3, the optimizer should focus on f1 only,
producing baseline-like task selection (priorities drive everything).
"""
...
class SARSchedulingProblemNoPhysics(Problem):
    """Variant D: f2 = f3 = 1.0 (constant per-task, after mean normalization).

    For each selected task, f2_num and f3_num accumulate 1.0; dividing by
    n_selected yields the constant 1.0 for every feasible schedule. The
    optimizer therefore receives no geometric signal in f2/f3 - only f1
    (profit) drives selection. This is the strongest ablation: it tests
    whether the NSGA-III solver's behavior is attributable to the physical
    content of f2/f3 or merely to the presence of extra objectives.
    """
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        inst = self.instance
        f1 = np.zeros(n_pop)
        f2_num = np.zeros(n_pop)   # VARIANT: count only
        f3_num = np.zeros(n_pop)   # VARIANT: count only
        ...
        for p in range(n_pop):
            ...
            for i in range(N):
                if selected[i]:
                    ...
                    f1[p] += task.priority
                    n_sel[p] += 1
                    # NO physics: f2 and f3 just count selections
                    f2_num[p] += 1.0
                    f3_num[p] += 1.0

                    # Still need to record φ and ψ for constraint checking
...
def moea_solver_no_physics(windows, targets, **kwargs):
    ...
    if instance.N == 0:
        return SolverResult(schedule=(), score=0.0, metadata={"solver": "moea_nsga3_no_physics", "n_tasks": 0, "frontier": []})

    f1_gbl = kwargs.get("f1_gbl", 1.0)
    instance.f1_gbl = f1_gbl
    problem = SARSchedulingProblemNoPhysics(instance, n_obj=n_obj, f1_gbl=f1_gbl)
    ...
    return SolverResult(
        ...
        metadata={
            "solver": "moea_nsga3_no_physics", "n_tasks": instance.N,
            ...
            "n_obj": n_obj, "ablation_variant": "D_no_physics",
...
def run_one(pkl_path):
    ...
    return {
        ...
        "n_obj": 3, "solver": "c2_moea_3obj_no_physics",
        "ablation_variant": "D_no_physics", "solver_version": GIT_COMMIT,
...
def main():
    ...
    print(f"MOEA-3obj NO-PHYSICS ablation (variant D): {len(groups)} groups, {total} scenarios")
