#!/usr/bin/env python3
"""Phase 7b: Run MOEA-2obj (MOEA-2: f1 profit + f2 geometric resolution).

Objectives: f1 (coverage profit) + f2 (geometric resolution, Σ sinθ·cosψ_sq).
f3 (NESZ radiometric) is computed post-hoc and included in output metadata.

Uses NSGA-III with population=100, generations=200, n_obj=2.
Saves incremental progress to experiments/results/moea_2obj/_progress.json.
"""
...
RESULTS_DIR = PROJECT / "experiments" / "results" / "moea_2obj"
...
# Slew params matching existing scenarios
SLEW_RATE = 0.0524  # rad/s (~3 deg/s)
...
def run_one(pkl_path: Path) -> dict:
    """Run MOEA-2obj (MOEA-2: f1+f2 geometric resolution) on a single scenario. Returns result dict."""
    ...
    f1_norm = float(meta.get("f1", 0.0))  # normalized (f1_raw / f1_gbl)
    f1_raw = float(meta.get("f1_raw", 0.0))  # actual coverage profit
    f1_gbl = float(meta.get("f1_gbl", 1.0))
    f2 = float(meta.get("f2", 0.0))  # mean geometric resolution
    f3 = float(meta.get("f3", 0.0))  # mean NESZ radiometric (post-hoc in 2obj)
    n_selected = int(meta.get("n_selected", 0))

    # Convert frontier to JSON-safe format
...
def main():
    ...
    parser.add_argument("--groups", nargs="+", help="Groups to process (e.g., S1 S2 S3)")
    parser.add_argument("--resume", action="store_true", default=True,
                        help="Resume from existing progress (default: True)")
    parser.add_argument("--no-resume", dest="resume", action="store_false",
                        help="Start fresh, overwriting existing progress")
    ...
    print(f"MOEA-2obj experiment: {len(groups)} groups, {total_scenarios} total scenarios")
    print(f"  Objectives: f1 (profit) + f2 (geometric resolution), f3 post-hoc")
    ...
    for group_name, files in groups.items():
        ...
        if grp_keys:
            ...
            frontier_sizes = [completed[k]["n_frontier"] for k in grp_keys]
            ...
            print(f"     f1={np.mean(f1s):.1f}+-{np.std(f1s):.1f}, "
                  ...
                  f"f3={np.mean(f3s):.1f}+-{np.std(f3s):.1f}, "
                  f"n_selected={np.mean(ns):.1f}+-{np.std(ns):.1f}, "
                  f"frontier={np.mean(frontier_sizes):.1f}+-{np.std(frontier_sizes):.1f}, "
                  f"time={np.mean(rts):.1f}s")

    # Final save with stats
    progress["stats"] = {
